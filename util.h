...
// MaxWu char *path_to_modname(const char *path, char buf[static PATH_MAX], size_t *len) __attribute__((nonnull(2)));
...
// MaxWu char *path_make_absolute_cwd(const char *p) _must_check_ __attribute__((nonnull(1)));
// MaxWu int mkdir_p(const char *path, int len, mode_t mode);
// MaxWu int mkdir_parents(const char *path, mode_t mode);
